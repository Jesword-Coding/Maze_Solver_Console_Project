...
namespace Maze_Game
{
    public class Artificial_Intelligence
    {
        ...
        private void TraceBackSolution(List<Maze> solution, Maze m)
        {
            ...
            var pair = new List<Tuple<int, int>>(); 
            Maze maze = m;
            solution.Add(m);
            ...
            while(maze.parent != null)
            {
                ...
                pair.Add(Tuple.Create(currentPosition[0], currentPosition[1])); 
                solution.Add(maze);
            }
        ...
        public void BFS(Maze initialState)
        {
            ...
            {
                Maze node = (Maze)queue.Dequeue();          // popping the first element off the queue
                node.PrintMaze();                           // explore that node and then add it to  
        ...
        public void AStar(Maze initialState)
        {
            ...
            while(pq.Count > 0 && !goal)
            {
                ...
                Console.WriteLine("\nG of n: " + node.Gof);
                Console.WriteLine("H of n: " + node.Hof);
